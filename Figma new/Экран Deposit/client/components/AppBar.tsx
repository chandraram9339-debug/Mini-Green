...
export function AppBar({ title, onBack, notificationCount = 3 }: AppBarProps) {
  return (
    <div className="w-full bg-[#ECF1F4] h-14 flex-shrink-0 relative flex items-center px-4">
      {/* Back Button */}
      <button
        onClick={onBack}
        className="flex items-center justify-center w-8 h-8 rounded-[30px] -rotate-90 flex-shrink-0"
      >
      ...
      {/* Title */}
      <h1 className="absolute left-1/2 -translate-x-1/2 text-[#192B48] text-xl font-medium tracking-[0.4px] font-outfit">
        {title}
      </h1>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="relative w-6 h-6 flex items-center justify-center">
          ...
          {notificationCount > 0 && (
            <div className="absolute -top-0.5 -right-1 w-[13px] h-[13px] bg-[#2D6E93] rounded-full flex items-center justify-center">
              <span className="text-white text-[7px] font-medium leading-none">{notificationCount}</span>
            </div>
          )}
        </div>
      ...
      {/* Bottom border */}
      <div className="absolute bottom-0 left-4 right-4 h-px bg-[#D3DAE5]" />
    </div>
  );
}
