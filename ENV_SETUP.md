...
## Сид-фраза (Settings → Seed) в мини-аппе

В ответе `GET /wallet` теперь есть `seedScreen: { mode, reason? }`; фраза словами — только в `GET /wallet/seed`.

| Что нужно на бэкенде | Поведение |
|---|---|
| **`WALLET_SEED_PER_USER=1`** и **`USER_WALLET_ENCRYPTION_KEY=`** (ровно **64 hex**) | Новые пользователи создаются с BIP39 (12 слов), сид показывается в мини-аппе (`mode: per_user`). |
| **`WALLET_SEED_PER_USER=0`** (дефолт в примере) | `mode: disabled`, `reason: feature_off` — сида **нет** у всех; это не баг фронта. |
| **`WALLET_STORE_ENCRYPTED_PK=1`** | Имеет **приоритет** над сидом для **новых** пользователей: кладётся зашифрованный приватник, `mode: custodial_pk` — фраза **не** показывается (custodial). Не включайте оба сценария одновременно, если нужна именно мнемоника. |
| Учётка создана **до** включения per-user seed или по **детерминированному** пути | `wallet_mnemonic_encrypted` пустой → `legacy` / `legacy_no_mnemonic` — **задним числом** фразу не восстановить без смены адреса депозита (опасно). |

После правок `.env` на сервере: **`pm2 restart miniapp-backend`**.
...
**Как сгенерировать** (12 слов BIP39): из корня репозитория после `pnpm install` (зависимость `bip39` уже в `backend`):
```bash
pnpm --dir backend exec node -e "import('bip39').then((m) => console.log(m.generateMnemonic()))"
```

> Или используй любой **офлайн** BIP39 генератор: https://iancoleman.io/bip39/ (выбрать TRON / 12 слов)
...
```env
TRON_API_KEY=<ваш_ключ_из_dashboard_trongrid>
TRON_FULL_HOST=https://api.trongrid.io
USDT_TRC20_CONTRACT=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t
LIVE_TRON=1
LIVE_TRON_SEND=1
JWT_ACCESS_TTL_SEC=31536000
...
### Если в git попали логи IDE или отладочные дампы

Удалите их из индекса, расширьте `.gitignore` (см. корневой файл), при необходимости очистите историю (`git filter-repo` / BFG) и выполните **ротацию** любых ключей, которые теоретически могли оказаться в этих файлах (скан `gitleaks` / `trufflehog`).
