...
export default function AppBar({ title, backTo, notificationCount = 25 }: AppBarProps) {
  return (
    <div className="relative flex items-center justify-between px-4 h-14 w-full">
      {/* Back button */}
      <Link
        to={backTo ?? "/"}
        className="flex items-center justify-center w-8 h-8 rounded-[30px] -rotate-90"
      >
        <BackIcon />
      </Link>
      ...
      {/* Title */}
      <h1
        className="absolute left-1/2 -translate-x-1/2 text-white text-xl font-medium"
